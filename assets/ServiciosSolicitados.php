...
$query = "SELECT s.IdSolicitud, s.ClienteId, t.NombreServicio, s.CodigoPostalId, cp.Municipio, cp.Asenta, s.Calle, s.NumExt, s.NumInt, s.EstadoSolicitud, s.FechaSolicitud,
                 c.Nombre, c.ApPaterno, c.ApMaterno, c.NumeTel, c.CorreoElectronico, sv.Fotos 
          FROM Solicitud s
          ...
          LEFT JOIN servicio sv ON s.IdSolicitud = sv.SolicitudId
        ...
        <tbody>
            <?php foreach ($solicitudes as $solicitud): ?>
                <tr>
                    ...
                    <td>
                        <?php if ($solicitud['EstadoSolicitud'] == 'Finalizado'): ?>
                <button onclick="openModal(<?php echo htmlspecialchars($solicitud['IdSolicitud']); ?>, '<?php echo htmlspecialchars($solicitud['Fotos']); ?>')">Pagar</button>
            <?php endif; ?>
                    </td>
                </tr>
            <?php endforeach; ?>
...
<script>
    ...
    function openModal(idSolicitud, foto) {
        document.getElementById("modalIdSolicitud").value = idSolicitud;
        var modalFotoContainer = document.getElementById("modalFotoContainer");
        modalFotoContainer.innerHTML = ''; // Clear any existing content
        ...
        if (foto) {
            ...
            modalFotoContainer.appendChild(img);
        }
